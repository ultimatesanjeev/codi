...
//==============================================		
...

			{
			 ...
			 $colorbook_cat_array = array();
			  ...
			  $outh=urldecode($this->input->get_post('outh'));

			    $imei=urldecode($this->input->get_post('imei'));	
...
exec("/usr/bin/convert ".UPLOADBASEIMAGE.$img_sketch." -colorspace gray \( +clone -tile pencil_tile.gif -draw color 0,0 reset \+clone +swap -compose color_dodge -composite \) \-fx 'u*.2+v*.8' -resize 1240x1240 ".UPLOADBASEIMAGE.$img_sketch);
							...
							exec("/usr/bin/convert ".UPLOADBASEIMAGE.$img_detail."  -modulate 100,0 \ \( +clone -blur 0x50 +level 1%x100% \) \ +swap -compose divide -composite -blur 10x2 -linear-stretch 7x0% -threshold 70% +dither -colors 2 -type bilevel -resize 1240x1240 ".UPLOADBASEIMAGE.$img_detail);
...
+swap -compose divide -composite -blur 20x5 -linear-stretch 7x0% -threshold 45% +dither -colors 2 -type bilevel -resize 1240x1240 ".UPLOADBASEIMAGE.$img_artistic);
							$output['artistic']=VIEWBASEIMAG.$img_artistic;
...
+swap -compose divide -composite -blur 80x30 -linear-stretch 7x0% -threshold 45% +dither -colors 2 -type bilevel -resize 1240x1240 ".UPLOADBASEIMAGE.$img_water);
							$output['water']=VIEWBASEIMAG.$img_water;
